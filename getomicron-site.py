...
def write_omicron_table(omicron_table):
    if args.verbose:
        print(f'Writing the omicron table for segment [{args.start},{args.end}] seconds.')
    ...
    # ---- Remote copy ----
    remote_user = "charliewilliam.winborn"
    remote_host = "ldas-pcdev6.ligo.caltech.edu"
    remote_dir = "~/projects/glitch/omicron_project/temp_csvs"
    remote_file = f"{remote_dir}/{os.path.basename(filename)}"

    scp_cmd = ["scp", filename, f"{remote_user}@{remote_host}:{remote_file}"]

    if args.verbose:
        print("Copying CSV to remote cluster:", " ".join(scp_cmd))
    ...
    try:
        ...
        print("Unexpected error during SCP:", e)
    # ----------------------
